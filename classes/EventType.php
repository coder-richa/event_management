...
class EventType extends Model {
    ...
    var $type_id, $title;
    static $table = EVENT_TYPE_TBL;

    function __construct($title="", $type_id = NULL) {
    ...
    static function getALL($title = "") {
        ...
        while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
            $data[] = new EventType($row['title'], $row['type_id']);
        }
        $db->closeConnection();
        return $data;
    }
    ...
    function save() {
        ...
        $isAddOperation = $this->getType_id() < 1;
        if (!(is_null($existingRecord) || ($existingRecord != NULL && $this->getType_id() == $existingRecord->getType_id()))) {
            $this->setType_id(-1);
            return;
        }
        $db = new DBHelper();
        if ($isAddOperation) {
            ...
            $this->setType_id($db->insert($query, $param));
        } else {
            $query = "UPDATE " . self::$table . " SET title=? "
                    . "WHERE type_id = ?";
            $param = array($this->getTitle(), $this->getType_id());
   ...
   static function delete($id) {
        $db = new DBHelper();
        $query = "DELETE FROM " . self::$table ." WHERE type_id = ?";
        $param = array($id);
        $db->update_delete($query, $param);
        $db->closeConnection();
    }
    ...
    static function getByTitle($title) {
        ...
        if ($row = $result->fetch(PDO::FETCH_ASSOC)) {
            $rec = new EventType($row['title'], $row['type_id']);
        }
        $db->closeConnection();
        return $rec;
    }
    ...
    static function getTableData($title = "") {
        ...
        foreach ($records as $key => $record) {
            $data[] = array($key + 1, $record->getTitle(), 
                self::createSpanWithLink($record->getType_id(), EVENT_TYPE_SAVE_FORM_API_URL),
                self::createSpanWithLink($record->getType_id(), EVENT_TYPE_DELETE_API_URL,  "Delete", "delete"));
        }
    ...
    static function saveFormData($id = 0) {
        ...
        return self::createAddEditTitleForm(EVENT_TYPE_SAVE_API_URL, $id, $title);
    }
